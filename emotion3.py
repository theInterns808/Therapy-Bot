...
def record_audio(duration=3, sample_rate=44100):
    ...
    # recording = mic.record(samplerate=sample_rate, numframes=sample_rate*duration)
    recording = mic.record(samplerate=sample_rate, numframes=sample_rate*duration, channels=1)
...
#        from emotion_recognition import EmotionRecognizer
        ...
        def custom_record_audio():
            return record_audio()
        
        rec = EmotionRecognizer(model=my_model, emotions=['sad', 'neutral', 'happy'], balance=True, verbose=0)
        
        
        rec.record_audio = custom_record_audio()
        
        rec.train()
        print("Test score:", rec.test_score())
        print("Train score:", rec.train_score())
        print("Recording a 3-second audio sample for real-time prediction...")
        
        pickle.dump(rec, open("/home/anrric/Desktop/TherapyMultiModalModel/speech_model.pkl", "wb"))
        
        loaded_model = pickle.load(open("/home/anrric/Desktop/TherapyMultiModalModel/speech_model.pkl", "rb"))
        
        # Save as WAV file
        with wave.open("test_recording1.wav", 'wb') as wf:
            wf.setnchannels(1)
            wf.setsampwidth(2)  # 2 bytes for 'int16' dtype
            wf.setframerate(44100)
            wf.writeframes(record_audio()) 
        
        emotion = rec.predict("test_recording1.wav")
        print(f"Predicted emotion: {emotion}")
        # pickle.dump(rec, open("/home/anrric/Desktop/TherapyMultiModalModel/model1", 'wb'))
